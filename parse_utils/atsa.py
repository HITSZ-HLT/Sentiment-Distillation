...
def parse_aspect_polarity(atsa_seq, sentence):
    ...
    def parse_seq(aspect_polarity):
        if type(aspect_polarity) in (tuple, list):
            aspect, polarity = aspect_polarity
        else:
            # aspect, polarity = aspect_polarity.split(',')
            aspect, polarity = re.split(r',|-|\|', aspect_polarity)
...
def parse_aspect_polarity_for_true(atsa_seq, sentence):
    aspect_polarity_list = []

    valid_flag = True

    for sub_seq in atsa_seq:

        aspect_polarity_list.append(tuple(sub_seq))

    # print(aspect_polarity_list)
...
class Result_atsa:
    ...
    def parse_from(cls, outputs):
        ...
        for example, prediction in outputs:
            ...
            aspect_polarity_list_true = parse_aspect_polarity_for_true(example['label_seq'], sentence)[0]
            aspect_polarity_list_pred = parse_aspect_polarity(prediction, sentence)
#                'original_output_of_model':original_prediction,


            print(aspect_polarity_list_true)
            print(aspect_polarity_list_pred)
            print('-'*100)
            ...
            data[ID] = {
                ...
                'golden_label_parsed': aspect_polarity_list_true,
                'prediction_parsed': aspect_polarity_list_pred,
